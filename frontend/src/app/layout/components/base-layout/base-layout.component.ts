...
@Component({
  ...
  template: `
    <div class="h-screen w-screen bg-gray-900 flex flex-col md:flex-row overflow-hidden" [style.--sidebar-width]="isSidebarOpen() ? '16rem' : '0rem'">
      <!-- Desktop Sidebar -->
      <aside class="hidden md:flex flex-col glass border-r border-gray-700/50 transition-all duration-300"
             [ngClass]="isSidebarOpen() ? 'w-64' : 'w-0 border-none'">
        <div class="p-6 flex items-center justify-between min-w-[256px]">
          <h1 class="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-purple-500">
            Training App
          </h1>
        </div>
        
        <nav class="flex-1 px-4 space-y-2 mt-4 min-w-[256px]">
        ...
        <div class="p-4 border-t border-gray-700/50 min-w-[256px]">
      ...
      <!-- Main Content Area -->
      <main class="flex-1 relative overflow-y-auto bg-gray-900 pb-16 md:pb-0 flex flex-col">
        <!-- Header for hamburger (desktop only) -->
        <header class="hidden md:flex items-center p-4 border-b border-gray-800 bg-gray-900/50 backdrop-blur sticky top-0 z-50">
          <button (click)="toggleSidebar()" class="p-2 rounded-lg text-gray-400 hover:text-white hover:bg-gray-800 transition-colors" title="Toggle Sidebar">
