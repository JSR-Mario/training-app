  ...
  template: `
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        ...
        <!-- Streak Card -->
        <div class="solid-card p-6 flex flex-col justify-between h-full col-span-1 min-w-0">
          <div class="flex items-center justify-between mb-4 gap-2">
            <h3 class="text-base font-medium text-gray-500 dark:text-gray-400 truncate">Activity Streak</h3>
            <div class="w-8 h-8 rounded-lg bg-accent-pos/20 flex items-center justify-center shrink-0">
          ...
          <div class="min-w-0 flex-1 flex flex-col justify-end">
            @if (isLoading()) {
              ...
              <p class="text-3xl sm:text-4xl lg:text-3xl xl:text-4xl font-bold tracking-tight text-black dark:text-white mb-3 mt-2 truncate">
                {{ summary()?.streak?.currentStreak || 0 }} <span class="text-lg sm:text-xl text-gray-500 dark:text-gray-400 font-normal">days</span>
              </p>
              <div class="flex items-center text-sm min-w-0">
                <span class="text-gray-500 dark:text-gray-400 truncate">Best: {{ summary()?.streak?.longestStreak || 0 }} days</span>
              </div>
            }
          </div>
        </div>
        ...
        <div 
          class="solid-card p-6 flex flex-col justify-between h-full col-span-1 min-w-0 hover:bg-gray-100 dark:hover:bg-gray-900 cursor-pointer transition-colors"
        ...
        >
          <div class="flex items-center justify-between mb-4 gap-2">
            <h3 class="text-base font-medium text-gray-500 dark:text-gray-400 truncate">Weights (Week)</h3>
            <div class="w-8 h-8 rounded-lg bg-accent-pos/20 flex items-center justify-center shrink-0">
          ...
          <div class="min-w-0 flex-1 flex flex-col justify-end">
            @if (isLoading()) {
              ...
              <p class="text-3xl sm:text-4xl lg:text-3xl xl:text-4xl font-bold tracking-tight text-black dark:text-white mb-3 mt-2 truncate">
                {{ (summary()?.weights?.volumeThisWeekKg || 0) | number:'1.0-0' }} <span class="text-lg sm:text-xl text-gray-500 dark:text-gray-400 font-normal">kg</span>
              </p>
              <div class="flex items-center text-sm min-w-0">
                @if ((summary()?.weights?.volumePercentageChange || 0) >= 0) {
                  <span class="text-accent-pos flex items-center bg-accent-pos/10 px-1.5 py-0.5 rounded text-xs font-medium whitespace-nowrap truncate">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 mr-1 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 10l7-7m0 0l7 7m-7-7v18" />
                    </svg>
                    {{ summary()?.weights?.volumePercentageChange | number:'1.0-1' }}%
                  </span>
                } @else {
                  <span class="text-accent-neg flex items-center bg-accent-neg/10 px-1.5 py-0.5 rounded text-xs font-medium whitespace-nowrap truncate">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 mr-1 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        >
          <div class="flex items-center justify-between mb-4 gap-2">
            <h3 class="text-base font-medium text-gray-500 dark:text-gray-400 truncate">Cardio (Week)</h3>
            <div class="w-8 h-8 rounded-lg bg-accent-pos/20 flex items-center justify-center shrink-0">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-accent-pos" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div class="min-w-0 flex-1 flex flex-col justify-end">
            @if (isLoading()) {
              ...
              <p class="text-3xl sm:text-4xl lg:text-3xl xl:text-4xl font-bold tracking-tight text-black dark:text-white mb-3 mt-2 truncate">
                {{ summary()?.cardio?.minutesThisWeek || 0 }} <span class="text-lg sm:text-xl text-gray-500 dark:text-gray-400 font-normal">min</span>
              </p>
              <div class="flex items-center text-sm min-w-0">
        ...
        >
          <div class="flex items-center justify-between mb-4 gap-2">
            <h3 class="text-base font-medium text-gray-500 dark:text-gray-400 flex flex-wrap items-center gap-1.5 min-w-0">
              <span>Body Weight</span>
              <span class="text-[10px] uppercase font-bold text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 rounded whitespace-nowrap">
                {{ summary()?.bodyWeight?.timeframeLabel || 'Since start' }}
              </span>
            </h3>
            <div class="w-8 h-8 rounded-lg bg-accent-pos/20 flex items-center justify-center shrink-0">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-accent-pos" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div class="min-w-0 flex-1 flex flex-col justify-end">
            @if (isLoading()) {
              ...
              <p class="text-3xl sm:text-4xl lg:text-3xl xl:text-4xl font-bold tracking-tight text-black dark:text-white mb-3 mt-2 truncate">
                {{ (summary()?.bodyWeight?.currentWeightKg || 0) | number:'1.0-1' }} <span class="text-lg sm:text-xl text-gray-500 dark:text-gray-400 font-normal">kg</span>
              </p>
              <div class="flex items-center text-sm min-w-0">
                @if (bodyWeightChangeStatus() === 'NEUTRAL') {
                  <span class="text-gray-500 dark:text-gray-400 flex items-center bg-gray-200 dark:bg-gray-700/50 px-1.5 py-0.5 rounded text-xs font-medium whitespace-nowrap truncate">
