...
@WebMvcTest(DayTemplateController.class)
@AutoConfigureMockMvc(addFilters = false)
class DayTemplateControllerTest {
    ...
    @MockBean
    private DayTemplateService dayTemplateService;

    @Test
    void createDay_Success() throws Exception {
        UUID userId = testUserId;
        UUID weekId = UUID.randomUUID();
        DayTemplateRequest req = new DayTemplateRequest("Push Day");
        DayTemplateResponse resp = new DayTemplateResponse(UUID.randomUUID(), weekId, "Push Day");

        Mockito.when(dayTemplateService.create(eq(userId), eq(weekId), any())).thenReturn(resp);

        mockMvc.perform(post("/api/v1/training/weeks/{weekId}/days", weekId)
